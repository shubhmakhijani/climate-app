...
const App = () => {
  ...
  useEffect(() => {
    const generateData = () => {
      ...
      let currentTemp = -0.4;
      let currentCO2 = 295;
      let currentRainfall = 1000;
      let currentAQI = 20;

      for (let year = startYear; year <= endYear; year++) {
        const t = (year - startYear) / (endYear - startYear);
        const acceleration = Math.pow(t, 2);
        
        currentTemp += (0.005 + acceleration * 0.03) + (Math.random() - 0.5) * 0.15;
        currentCO2 += (0.5 + acceleration * 2.5) + (Math.random() - 0.5) * 1.5;
        currentRainfall += (Math.random() - 0.5) * 40 - (acceleration * 5);
        currentAQI += (0.1 + acceleration * 0.8) + (Math.random() - 0.5) * 3;

        const seasonal = Array.from({ length: 12 }, (_, i) => ({
          month: ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'][i],
          tempDeviation: Math.sin((i - 3) * (Math.PI / 6)) * 12 + (Math.random() - 0.5),
          rainVolume: Math.max(0, 80 + Math.cos((i - 1) * (Math.PI / 6)) * 40 + (Math.random() - 0.5) * 20),
          aqiLevel: Math.max(10, currentAQI + Math.sin((i - 6) * (Math.PI / 6)) * 15 + (Math.random() - 0.5) * 5)
        }));
        ...
        historicalData.push({
          ...
          rainfall: Number(currentRainfall.toFixed(1)),
          aqi: Number(Math.max(10, currentAQI).toFixed(1)),
          seasonal,
          sma10_temp: null
        });
      }
      ...
      for (let i = 9; i < historicalData.length; i++) {
        let sum = 0;
        for (let j = 0; j < 10; j++) {
          sum += historicalData[i - j].temp;
        }
        historicalData[i].sma10_temp = Number((sum / 10).toFixed(2));
      }
  ...
  const seasonalData = useMemo(() => {
    ...
    const recentYears = data.slice(-20);
    return Array.from({ length: 12 }, (_, i) => {
      const monthData = recentYears.map(y => y.seasonal[i]);
      return {
        month: monthData[0].month,
        avgTemp: Number((monthData.reduce((a, b) => a + b.tempDeviation, 0) / 20).toFixed(2)),
        avgRain: Number((monthData.reduce((a, b) => a + b.rainVolume, 0) / 20).toFixed(2)),
        avgAqi: Number((monthData.reduce((a, b) => a + b.aqiLevel, 0) / 20).toFixed(2))
      };
    });
  }, [data]);

  const mlForecastData = useMemo(() => {
    if (!data.length || activeModule !== 'ml') return [];
  ...
  const dlForecastData = useMemo(() => {
    if (!data.length || activeModule !== 'dl' || !dlTrained) return [];
    const results = data.map(d => ({ ...d, historical: d[targetVariable] }));
    const last20 = data.slice(-20);
    const trend = (last20[19][targetVariable] - last20[0][targetVariable]) / 20;
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-950 text-white">
        ...
        <p className="mt-4 text-slate-400 font-mono text-sm animate-pulse">Aggregating Global Climate Repositories...</p>
      </div>
    );
  }
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
              <div className="flex justify-between items-end mb-6">
                <div>
                  ...
                  <p className="text-slate-500 text-sm mt-1">Averaged over the trailing 20-year window</p>
                </div>
              </div>
